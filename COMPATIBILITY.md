...
This file is the published compatibility contract for **BDVM.Full 0.3.0 beta**. It describes the current incremental release; it is not a promise that BDVM is feature-complete.
...
| External runtime dependencies | BDVM Multiplayer fork with `MultiplayerAPI` 1.4.0+, SelfShunt, PassengerJobs, PassengerJobs.API, RemoteDispatchLive; DVLangHelper transitively | A missing dependency disables its dependent bridge or feature. Multiplayer API older than 1.4.0 is refused because remote individual-wallet settlement cannot be guaranteed. |
