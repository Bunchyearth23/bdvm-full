...
| Current declared requirements | `Multiplayer`, `SelfShunt` |
| Release state | Development build; packaging and compatibility matrix are not final |

## Included modules

The bundle references `BDVM.Common`, `BDVM.Core`, `BDVM.Companies`, `BDVM.Fleet`, `BDVM.Market`, `BDVM.Operations`, `BDVM.Passengers`, `BDVM.Web`, `BDVM.Dispatch`, `BDVM.Management`, `BDVM.MultiplayerBridge` and `BDVM.SelfShuntBridge`.
...
- The current `info.json` requires Multiplayer and SelfShunt because their bridges are included. The architecture permits omitting those bridges later, but this bundle does not yet package that choice.
...
All 13 `BDVM.*` repositories must be siblings under `src/`. Building requires a Derail Valley installation for Unity, game and Unity Mod Manager assemblies. It also requires a compatible `MultiplayerAPI.dll` and the authorized SelfShunt fork/API at the paths expected by the project.
